...
for desc1, row1 in zip(descriptions1, data1):
    matches = difflib.get_close_matches(desc1, descriptions2, n=1)
    if matches:
        desc2 = matches[0]
...
with open('output.csv', 'w', newline='') as output_file:
    ...
    for row1, similarity in similarities:
        for row2 in data2:
            if row2['description'].lower() == difflib.get_close_matches(row1['Description'].lower(), descriptions2, n=1)[0]:
